...
input_file = "T_AISearchfile100.txt"
#input_file = "a"
...
def gen_init(matrix, city_num):
    new_tour = random.sample(range(0, city_num), city_num)
    new_score = score(matrix, new_tour)
    init = (new_tour, new_score)
    return init


# Mutates (AKA swaps two cities).
def mutate(matrix, city_num, tour):
    new_tour = copy.deepcopy(tour)
    swap_a, swap_b = random.randint(0, city_num - 1), random.randint(0, city_num - 1)
    minus = matrix[new_tour[0][swap_a]][new_tour[0][swap_a-1]] + matrix[new_tour[0][swap_a]][new_tour[0][(swap_a+1) % (city_num)]] + matrix[new_tour[0][swap_b]][new_tour[0][swap_b-1]] + matrix[new_tour[0][swap_b]][new_tour[0][(swap_b+1) % (city_num)]]
    new_tour[0][swap_a], new_tour[0][swap_b] = new_tour[0][swap_b], new_tour[0][swap_a]
    add = matrix[new_tour[0][swap_a]][new_tour[0][swap_a - 1]] + matrix[new_tour[0][swap_a]][new_tour[0][(swap_a + 1) % (city_num)]] + matrix[new_tour[0][swap_b]][new_tour[0][swap_b - 1]] + matrix[new_tour[0][swap_b]][new_tour[0][(swap_b + 1) % (city_num)]]
    new_score = tour[1] - minus + add
    return new_tour[0], new_score


# Probability of accepting successor happening anyway.
def prob(temp, diff, beta):
    gofx = temp / ((diff ** 2 + temp ** 2) ** ((beta + 1)/2))
    if random.random() < gofx:
        return True
    else:
        return False


# Creates current and successor
def compare(current, succ, temp, beta):
    diff = current[1] - succ[1]
    if diff >= 0:
        return succ
    else:
        change = prob(temp, diff, beta)
        if change:
            return succ
        else:
            return current
...
def main(matrix, city_num):
    ...
    current = gen_init(matrix, city_num)

    while temp > threshold:
        temp = schedule(temp, beta)
        succ = mutate(matrix, city_num, current)
        current = compare(current, succ, temp, beta)

        ### Added Break check
        if (time.time() - start) > timeout: temp = 0
