...
def gen_pop(matrix, city_num, size):
    ...
    for x in range(size):
        ...
        pop_score += new_score
    return pop, pop_score


# Picks a random tour from the population.
def rand_pop(pop, pop_score):
    rand = random.randint(1, pop_score)
    index = 0
    while rand > 0:
        rand -= pop[index][0]
        index += 1
    chosen_pop = pop[index - 1][1]
    return chosen_pop
...
def child(x, y, matrix, city_num, mutate_prob):
    split = random.randint(0, city_num)
    z1 = x[:split] + y[split:]
    z2 = y[:split] + x[split:]
...
def elitism(pop, pop_size, score, elite):
    pop.sort()
    for rem_score in pop[pop_size - elite:]:
        score -= rem_score[0]
...
def main(matrix, city_num):
    ...
    pop, pop_score = gen_pop(matrix, city_num, pop_size)
    for x in range(7000):
        new_pop = []
        new_pop_score = 0
        for count in range(pop_size):
            x = rand_pop(pop, pop_score)
            y = rand_pop(pop, pop_score)
            z = child(x, y, matrix, city_num, mutate_prob)

            new_pop.append(z)
            new_pop_score += z[0]

        pop, pop_score = elitism(new_pop, pop_size, new_pop_score, elite)
